...
const preview = read('viewer/src/pro/BaoPreview.tsx')
...
test('real Kanzlei Autopilot is mounted only inside authenticated Pro app', () => {
  ...
  assert.match(proApp, /path="friday"/)
  assert.match(proApp, /path="privacy"/)
  assert.match(proApp, /<ProAuth>/)
  assert.match(proApp, /BaoAutopilotDashboard/)
  assert.match(proApp, /BaoCaseWorkPacket/)
})

test('Bao public entry is synthetic-only and contains no operational case-store access or embedded pilot token', () => {
  assert.match(main, /path="\/bao" element={<BaoPreview/)
  assert.match(main, /path="\/willkommen\/bao" element={<BaoPreview/)
  assert.doesNotMatch(preview, /listCases|listResearch|getCase|updateCase|saveResearch|localStorage|sessionStorage/)
  assert.doesNotMatch(preview, /BETA-NGUYEN|betaToken|invite=/)
  assert.match(preview, /SYNTHETIC SANDBOX/)
  assert.match(preview, /keine echten Mandatsdaten/i)
  assert.match(preview, /Externe Nachrichten: <strong>0<\/strong>/)
  assert.match(preview, /Rechtliche Entscheidungen: <strong>0<\/strong>/)
  assert.doesNotMatch(preview, /VwVfG/)
})

...
import { AlertTriangle, Bot, CheckCircle2, Clock3, FileCheck2, Play, ShieldCheck, Users } from 'lucide-react'
...
import { runSafeKanzleiAutopilot, type SafeAutopilotRunReport } from './kanzlei-autopilot-runner'
...
const EMPTY_RUN: SafeAutopilotRunReport = {
  scannedCases: 0,
  ocrJobsQueued: 0,
  missingDocumentTasksCreated: 0,
  deadlineReviewTasksCreated: 0,
...
export default function BaoAutopilotDashboard() {
  ...
  const [lastRun, setLastRun] = useState<SafeAutopilotRunReport>(EMPTY_RUN)
  const [lastRunAt, setLastRunAt] = useState<string>('')

  const cases = useMemo(() => listCases(), [revision])
  const research = useMemo(() => listResearch(), [revision])
  const analytics = useMemo(() => getAnalyticsSnapshot(), [revision])
  ...
  function runNow() {
    const report = runSafeKanzleiAutopilot()
    setLastRun(report)
    ...
    <div className="space-y-8">
      <section>
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5 mt-4">
          <div>
            <h1 className="text-4xl md:text-5xl font-semibold" style={{ fontFamily: "'Georgia', serif" }}>Bao, heute brauchst du nur hierhin.</h1>
            <p className="text-lg text-[var(--color-ink-soft)] max-w-3xl mt-3 leading-relaxed">
              Der Autopilot bereitet interne Routinearbeit vor und sortiert sie nach Autorität. Du siehst zuerst nur Fristen, ungeprüfte Recherche und andere Punkte, die wirklich anwaltliche Aufmerksamkeit brauchen.
            </p>
          </div>
          <button onClick={runNow} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[var(--color-ink)] text-white px-4 py-2.5 font-semibold hover:opacity-90 shrink-0">
            <Play className="w-4 h-4" /> Sicheren Lauf starten
          </button>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[.16em] font-bold text-emerald-800">Letzter sicherer Lauf</p>
            <p className="text-sm text-emerald-950 mt-1">
              {lastRun.scannedCases} Akten geprüft · {lastRun.ocrJobsQueued} OCR-Jobs · {lastRun.missingDocumentTasksCreated} Unterlagen-Aufgaben · {lastRun.deadlineReviewTasksCreated} Frist-Reviews angelegt
            </p>
          </div>
          <div className="text-xs text-emerald-900/70 sm:text-right">
            <div>Externe Nachrichten: <strong>{lastRun.externalMessagesSent}</strong></div>
            <div>Rechtsentscheidungen: <strong>{lastRun.legalDecisionsMade}</strong></div>
            {lastRunAt && <div className="mt-1">{new Date(lastRunAt).toLocaleTimeString('de-DE')}</div>}
          </div>
        </div>
      </section>

      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <Metric icon={<FileCheck2 className="w-4 h-4" />} value={snapshot.activeCases} label="aktive Akten" />
        ...
        <Metric icon={<Bot className="w-4 h-4" />} value={snapshot.automaticQueue.length} label="noch vorbereitbar" />
      ...
      <section className="grid lg:grid-cols-[1.1fr_.9fr] gap-6">
        ...
        <div className="rounded-2xl border border-[var(--color-border)] bg-white p-5 h-fit">
          ...
          <div className="mt-5 rounded-xl bg-slate-50 border border-slate-200 p-4 text-xs text-slate-600 leading-relaxed">
            <strong>Wichtig:</strong> Diese Beta-Oberfläche zählt echte GitLaw-Pro-Zustände. Sie behauptet keine Zeitersparnis. Stunden werden erst veröffentlicht, wenn Bao die Vorher-/Nachher-Baseline bestätigt hat.
          </div>
          <div className="mt-4 text-xs"><Link to="/pro/akten" className="underline">Alle Akten öffnen →</Link></div>
        </div>
      </section>

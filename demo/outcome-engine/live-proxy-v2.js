(() => {
  ...
  style.textContent=`
    ...
    .backend-proof.fallback{border-color:rgba(168,91,0,.22);background:#fff7e8;color:#754800}.backend-proof.fallback strong{color:#8b5300}
  ...
  function updateMode(){
    ...
    privacy.textContent=live?'Live Server-Workflow · ausschließlich Demo-Daten':'Interaktiver Browser-Prototyp · keine Übertragung';
    privacy.classList.toggle('demo',!live);
    appStatus.innerHTML=live?'Kanzlei: <strong>Live Backend</strong> · kein automatischer Versand':'Produktprototyp · <strong>lokale Auswertung</strong> · keine Übertragung';
    els.runBtn.textContent=live?'Live analysieren →':'Outcome Engine starten →';
  }

  selectScenario=function(key){ originalSelectScenario(key); updateMode(); };
  resetResults=function(){
    originalResetResults();
    backendProof.className='backend-proof'; backendProof.innerHTML='';
    ...
    els.runBtn.textContent=currentKey==='legal'?'Live analysieren →':'Outcome Engine starten →';
  };

  function isoToGerman(iso){const m=String(iso||'').match(/^(\d{4})-(\d{2})-(\d{2})$/);return m?`${m[3]}.${m[2]}.${m[1]}`:iso;}
  async function runLive(text){
    const started=performance.now();
    const r=await fetch('/api/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({letter_text:text})});
    ...
    if(a.mandant_notification||a.anwalt_response)tasks.push('Entwurf menschlich prüfen');
    ...
    const mode=p.gateway?.backend_mode||'unknown';
    return {
      type:(c.letter_type?c.letter_type.replaceAll('_',' '):(meta.betreff||'Behördenkorrespondenz')),
      summary:p.final_message||c.reasoning_de||'Der Brief wurde strukturiert verarbeitet.',
      deadline:{date:f.deadline_iso?isoToGerman(f.deadline_iso):(f.raw_text||'Nicht eindeutig erkannt'),relative,missing},
      docs:docs.length?docs:['Keine ausdrücklich geforderten Unterlagen erkannt'],tasks,draft,
      urgency:c.urgency?c.urgency[0].toUpperCase()+c.urgency.slice(1):'Manuell prüfen',
      blocker:blockers.length>0,
      warning:blockers.length?`Sicher gestoppt: ${blockers.join(' · ')}.`:'Live-Ergebnis: Vor Freigabe gegen das Originaldokument prüfen.',
      live:true,
      backend:{runId:p.agent_run_id||'—',status:p.status||'completed',latencyMs:p.gateway?.elapsed_ms||Math.round(performance.now()-started),costUsd:Number(p.total_cost_usd||0),iterations:p.iterations||trace.length,trace,mode,agentError:p.gateway?.agent_error||null,source:p.gateway?.source||null}
    };
  }

  showResults=function(r){
    originalShowResults(r);
    const fallback=r.backend?.mode==='gitlaw_grounded_rules';
    els.confidence.textContent=fallback?'Live · regelbasiert':'GitLaw Agent live';
    if(r.backend){
      const b=r.backend;
      backendProof.className=`backend-proof visible${fallback?' fallback':''}`;
      const heading=fallback?'Live Workflow · GitLaw-Quellencheck · AI-Agent vorübergehend nicht verfügbar':'Live GitLaw Agent execution';
      const note=fallback?`<br><b>Transparent:</b> Der Agent-Key ist blockiert. Das Ergebnis stammt aus serverseitigen Regeln und einem Live-Quellencheck — nicht aus einer versteckten Simulation.`:'';
      backendProof.innerHTML=`<strong>${heading}</strong><br>Run ${escapeHtml(b.runId)} · ${escapeHtml(b.status)} · ${(b.latencyMs/1000).toFixed(1)} s · $${b.costUsd.toFixed(4)} · ${b.iterations} steps${note}<div class="trace-tools">${b.trace.map(t=>`<span class="trace-tool">${escapeHtml(t)}</span>`).join('')}</div>`;
    }
  ...
  runWorkflow=async function(){
    ...
    resetResults();els.runBtn.disabled=true;els.runBtn.textContent=currentKey==='legal'?'Backend verarbeitet live …':'Workflow läuft …';
    try{
  ...
  const fine=document.querySelector('.fineprint');fine.textContent='Der Kanzlei-Fall läuft über einen echten Server-Workflow, versucht zuerst den GitLaw-Agenten und fällt bei Provider-Ausfall transparent auf nachvollziehbare Extraktionsregeln plus Live-Quellencheck zurück. Andere Rollen bleiben Produktprototypen. Keine Nachricht wird automatisch versendet.';
  updateMode();
})();

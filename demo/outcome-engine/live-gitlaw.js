...
(() => {
  ...
  let liveSessionToken=sessionStorage.getItem('gitlaw_demo_session') || null;
  const GITLAW={baseUrl:'https://gitlaw-xi.vercel.app',invite:'DEMO',kanzleiName:'Outcome Engine Demo'};
  ...
  async function getSession(){
    ...
    const r=await fetch(`${GITLAW.baseUrl}/api/pro/session`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({invite:GITLAW.invite})});
    const b=await r.json().catch(()=>({}));
    if(!r.ok||!b.token)throw new Error(b.error||`Session fehlgeschlagen (${r.status})`);
    liveSessionToken=b.token;sessionStorage.setItem('gitlaw_demo_session',b.token);return b.token;
  }
  async function runLive(text){
    const started=performance.now();let token=await getSession();
    const call=()=>fetch(`${GITLAW.baseUrl}/api/agent/behoerden`,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${token}`},body:JSON.stringify({letter_text:text,kanzlei_name:GITLAW.kanzleiName})});
    let r=await call();
    if(r.status===401){sessionStorage.removeItem('gitlaw_demo_session');liveSessionToken=null;token=await getSession();r=await call();}
    const p=await r.json().catch(()=>({}));
    if(!r.ok)throw new Error(p.error||p.final_message||`Agent fehlgeschlagen (${r.status})`);
